...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for k,v in node2distances.items():
        if (node2distances[k] > max):
            max = node2distances[k]
    ...
    for k, v in node2distances.items():
        for i in range(max+1):
            if (node2distances[k] == i):
                level[i].append(k)
    
    
    for kp,vp in node2parents.items():
        value[kp] = 1.0
        if len(vp) > 1:
            value[kp] =  1/len(vp)
    ...
    for i in range(max-1, -1 , -1):
        for n in level[i]:
            for k, v in node2parents.items():
                if n in node2parents[k]:
                    value[n] += value[k]
                    result[(k,n)] = value[k]
                    
    r = defaultdict(list)
    
    for nodes,value in result.items():
        r[(tuple(sorted(nodes)))] = value
    return(r)
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        #print(node)
        a, b, c = bfs(graph, node, max_depth)
        b_u = bottom_up(node, a, b, c)
        #print(b_u)
        for key, value in b_u.items():
            a_b[key] += value/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    pass
    g = graph.copy()
    a_b = approximate_betweenness(g, max_depth)
    
    a_bsorted = sorted(a_b.items(), key = lambda x :(-x[-1],x[0]))
...
def norm_cut(S, T, graph):
    ...
    return float(c/volume_S + c/volume_T)
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
        ...
        nor_cut_val = norm_cut(components[0].nodes(), components[1].nodes(), graph)
        result.append((i, nor_cut_val))
...
def make_training_graph(graph, test_node, n):
    ...
    pass
    g = graph.copy()
    nodes = g.neighbors(test_node)

    a = sorted(nodes, key = str.lower)

    for i in range(n):
        g.remove_edge(test_node,a[i])
    return(g)
...
def jaccard(graph, node, k):
    ...
    scores1 = sorted(scores, key=lambda x: x[0])
    scores2 = sorted(scores1, key=lambda x: x[1], reverse=True)
    return scores2[:k]
    
#print(jaccard(training_graph, 'D', 2))
...
def path_score(graph, root, k, beta):
    ...
    #print(edges)
    for i in dist_list:
        if ((root,i[0]) not in edges and (i[0],root) not in edges and root != i[0]):
            scores.append(((root,i[0]),beta**i[1]*b[i[0]]))
