...
def token_pair_features(tokens, feats, k=3):
    ...
    for i in range(len(tokens) - k + 1):
        ...
        for i in l1:
            comb_list.append(i)
    c = dict(Counter(comb_list))
    for k, v in c.items():
        k1 = tuple(k)
        new_key = 'token_pair=' + k1[0] + '__' + k1[1]
        feats[new_key] = v
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for i in tokens_list:
        ...
        feature[count] = featurize(i, feature_fns)
        
        for val in feature[count]:
            if val[1] > 0:
    ...
    data1 = np.array(data, dtype='int64')
    row1 = np.array(row, dtype='int64')
    col1 = np.array(col, dtype='int64')
    
    mat = csr_matrix((data1,(row1, col1)))
    ...
    for k, v in sorted(feature.items()):
        for f in v:
            if f[0] in vocabulary:
                col.append(vocabulary[f[0]])
    # print(data)
    # print(row)
    # print(col)
    count = {}
    ...
    return mat[:, min_list], vocabulary
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    tokens_list = {}
    for i in punct_vals:
        tokens_list[i] = [tokenize(d, i) for d in docs]
    ...
    for i in x:
        b = b + 1
        if i[1] == True:
            mat, vocab = vectorize(tokens_list[True], i[0], i[2])
            cv = cross_validation_accuracy(LogisticRegression(), mat, labels, 5)
            result.append({'Features': (i[0]), 'Punct': True, 'min_freq': i[2], 'accuracy': cv})
        else:
            mat, vocab = vectorize(tokens_list[False], i[0], i[2])
            cv = cross_validation_accuracy(LogisticRegression(), mat, labels, 5)
            result.append({'Features': (i[0]), 'Punct': False, 'min_freq': i[2], 'accuracy': cv})

    return sorted(result,key =lambda d:d['accuracy'],reverse = True)
...
def plot_sorted_accuracies(results):
    ...
    graph = []
    for i in results:
        graph.append(i['accuracy'])
    plt.plot(sorted(graph))
    plt.xlabel('setting')
    plt.ylabel('accuracy')
...
def mean_accuracy_per_setting(results):
    ...
    mean_acc = defaultdict(list)
    li =[]
    
    for i in results:
        
        l = []
        key = 'min_freq='+str(i['min_freq'])
        mean_acc[key].append(i['accuracy'])
        key = 'punct='+str(i['Punct'])
        mean_acc[key].append(i['accuracy'])

        for f in i['Features']:
            l.append(str(f.__name__))
    ...
    return sorted(li, key=lambda k:k[0])[::-1]
...
def top_coefs(clf, label, n, vocab):
    ...
    clf_list = clf.coef_[0]
    if label == 1:
        clf_sorted_pos = np.argsort(clf_list)[::-1][:n]
        vb = []
        for i in clf_sorted_pos:
            for k,v in vocab.items():
                if v == i:
                    vb.append((k,clf_list[i]))
    elif label == 0:
        clf_sorted_neg = np.argsort(clf_list)[::1][:n]
        vb = []
        for i in clf_sorted_neg:
            for k,v in vocab.items():
                if v == i:
                    vb.append((k,abs(clf_list[i])))
    return sorted(vb,key = lambda x:x[1],reverse = True)
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for i in range(len(test_docs)):
        ...
        if test_labels[i] != predict_list[i]:
           di.update({'test_labels' : test_labels[i]})
           di.update({'predicted' : predict_list[i]})
           di.update({'proba' : proba_list[i][0]})
           di.update({'doc' : test_docs[i]})
           result.append(di)
    for i in range(len(test_docs)):
        di = {}
        if test_labels[i] != predict_list[i]:
           di.update({'test_labels' : test_labels[i]})
           di.update({'predicted' : predict_list[i]})
           di.update({'proba' : proba_list[i][1]})
           di.update({'doc' : test_docs[i]})
           result.append(di)
    result = sorted(result, key = lambda x: x['proba'], reverse = True)[:n]
        
    for i in result:
        print('truth='+str(i['test_labels'])+' prdicted='+str(i['predicted'])+' prob='+str(i['proba']))
        print(i['doc'])
        print(" ")
